...
[![Build Status](https://travis-ci.org/Partyschaum/express-node-typescript.svg?branch=master)](https://travis-ci.org/Partyschaum/express-node-typescript)

# Pre-reqs
- Install [Node.js](https://nodejs.org/en/)
- Install [MongoDB](https://docs.mongodb.com/manual/installation/) or [use a Docker image](https://store.docker.com/images/mongo)
...
> **Note!** Make sure you have already built the app using `yarn run build` or `yarn run build`
...
| package.json             | File that contains yarn dependencies as well as [build scripts](#what-if-a-library-isnt-on-definitelytyped)                          |
...
### Running the build
All the different build steps are orchestrated via [yarn scripts](https://docs.yarnjs.com/misc/scripts).
...
To call a script, simply run `yarn run <script-name>` (or `yarn run <script-name` if using yarn) from the command line.
...
These `.d.ts` files can be easily installed into your project by using the yarn scope `@types`.
For example, if we want the `.d.ts` file for jQuery, we can do so with `yarn add --dev @types/jquery`.

> **Note!** Be sure to add `--dev` (or `-D`) to your `yarn install`. `.d.ts` files are project dependencies, but only used at compile time and thus should be dev dependencies.
...
Once `.d.ts` files have been installed using yarn, you should see them in your `node_modules/@types` folder.
...
Likewise be sure to stop the debugger before returning to your normal `yarn start` process.
...
And last but not least I took [Microsofts TypeScript port of this](https://github.com/Microsoft/TypeScript-Node-Starter) and fiddled with it.
